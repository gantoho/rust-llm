...
enum SrcIdx {
    ...
    Map(Rc<Vec<usize>>),  // 通用情况：查预建的下标表（Rc 共享给反向闭包，不克隆）
}
...
fn binary(
    ...
    let a_view: SrcIdxView<'_> = (&a_src).into();   // Rc 不是 Sync，转成跨线程的切片视图
...
result.backward = Some(Rc::new(move || {
    ...
    let (ga, gb) = if same_shape && Rc::ptr_eq(&sg, &og) {
        ...
        for t in 0..g.len() {
            let ia = src_idx(t, &a_src_c);   // SrcIdx：Ident 取 t、Mod 取模、Map 查表
...
> 上面的建表与搬运是**原理示意**：真实实现把 map 缓存进 `PERMUTE_MAP_CACHE`（训练形状固定，只建一次、用 `Rc` 共享给反向闭包），输出索引用固定栈数组 `[0usize; 8]`，搬运数据按 4096 元素分块并行（`src/tensor.rs:653-731`）。
