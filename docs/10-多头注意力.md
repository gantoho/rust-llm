...
GQA 时先把 K/V 的头重复 `n_rep = n_head / n_kv_head` 次，让两边头数对齐（`repeat_kv`，`src/attention.rs:224`；标准 MHA 下 `n_rep = 1`，这一步跳过）：

```rust
// 4. 拆头之后：GQA 把 K/V 的每个头重复 n_rep 次
...
对齐后 [B*H, T, head_dim] 正好是 `matmul` 支持的 3D 批量形状，第 9 课的公式（打分 → 掩码 → softmax → 加权求和）被一个融合算子一次算完：
...
| ⑤ GQA 重复 K/V | `repeat_kv(&k, self.n_rep)` | K/V: [B*4, T_total, 16]（`n_rep` = 1 时跳过）|
...
cargo run --release -- demo    # 演示 3 训练小 GPT：4 头注意力参与每一轮前向/反向
```
