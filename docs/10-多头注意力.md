...
> 代码位置：[src/model.rs](file:///d:/Code/Rust/llm_from_scratch/src/model.rs)（`MultiHeadAttention` 结构 + `forward`）
> 底层算子：[src/tensor.rs](file:///d:/Code/Rust/llm_from_scratch/src/tensor.rs)（`reshape` / `permute` / `matmul` / `softmax_last_dim`）
