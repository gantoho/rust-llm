...
> 代码位置：[src/train.rs](file:///d:/Code/Rust/llm_from_scratch/src/train.rs)
...
对应代码（[src/train.rs](file:///d:/Code/Rust/llm_from_scratch/src/train.rs) 中的 `train_gpt`，省略了日志打印）：
...
这是自动微分（第 2 课）的核心。看 [src/tensor.rs](file:///d:/Code/Rust/llm_from_scratch/src/tensor.rs) 里的实现：
...
pub fn backward(&self) {
    ...
    fn dfs(t: &Tensor, order: &mut Vec<Tensor>, visited: &mut HashSet<usize>) {
        ...
        for p in t.parents.iter() { dfs(p, order, visited); }
        order.push(t.clone());
    }
...
代码（[src/train.rs](file:///d:/Code/Rust/llm_from_scratch/src/train.rs)）：
