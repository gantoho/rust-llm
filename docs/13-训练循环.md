...
> 上面是"最小可跑"的六步。项目当前的 `train_gpt` 在此基础上叠加了三项工程特性：
...
> 注意 `max_norm` 不是硬编码的，它来自 `TrainConfig.grad_clip`（`train_gpt` 里调用的是 `clip_grad_norm(&params, cfg.grad_clip)`）：仓库 `config/config.json` 里是 1.0，训练 LLM 时这是很常见的取值；而 `TrainConfig::default()` 给的是 1e6，相当于默认不裁剪。裁剪**不改变梯度方向**，只是限制步长上限——这比粗暴地调低学习率更精准。
...
`train_gpt` 每隔 `eval_every` 步打印一行（演示 3 的 `demo_gpt` 里 `eval_every = 100`）：
...
2. 把 `grad_clip` 从 1e6 改成 1.0 再训练，对比收敛速度（阈值太小会"拖慢"训练）。
