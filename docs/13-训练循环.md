...
> 注意 `max_norm` 不是硬编码的，它来自 `TrainConfig.grad_clip`（`train_gpt` 里调用的是 `clip_grad_norm(&params, cfg.grad_clip)`）：仓库 `config.json` 里是 1.0，训练 LLM 时这是很常见的取值；而 `TrainConfig::default()` 给的是 1e6，相当于默认不裁剪。裁剪**不改变梯度方向**，只是限制步长上限——这比粗暴地调低学习率更精准。
...
训练过程可以通过 `config.json` 的 `log_file` 字段启用 CSV 格式的指标日志：
