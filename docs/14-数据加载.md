...
`DataLoader::sample_batch` 的真实实现只有一行——它把训练区 `[0, val_start)` 交给通用的 `sample_region`（[src/data.rs](../src/data.rs)）。`val_start` 是 `DataLoader` 的字段，表示训练区与验证区的分界（用 `DataLoader::new` 时它等于 `tokens.len()`，即整段语料都当训练数据；用 `from_texts` 时由训练/验证文本切分得到）：
...
fn sample_region(&self, rng: &mut Rng, lo: usize, hi: usize, tag: &str) -> (Vec<usize>, Vec<usize>) {
    let max_start = hi - lo - self.block_size - 1;   // 至少留 1 个 token 当 y 的尾巴
...
`sample_region` 里 `max_start = hi - lo - block_size - 1`（`sample_batch` 传的区间是训练区 `[0, val_start)`）还额外要求至少留 1 个 token 当 y 的尾巴（因为 y 比 x 多取一位）。如果语料比 `block_size` 还短，就切不出任何完整窗口，直接 panic 提示——这是第 1 课就强调的"出错早、出错明显"的防御性编程习惯。

## 9. 动手练习

1. 手动算一遍：语料有 700 个 token，`block_size = 32`，`max_start` 是多少？一共能切出多少个不同的起点？（答案：`max_start = 700 - 0 - 32 - 1 = 667`；代码里 `start = lo + rng.choice(max_start)`，而 `choice(n)` 返回 `[0, n)`，所以实际能采到的是 `0..=666` 共 **667** 个起点。注：`start = 667` 数学上也合法（`x` 占 `667..699`、`y` 占 `668..700`），只是 `choice(max_start)` 取不到它——一处无害的 off-by-one。）
