...
>
> ⚠️ **历史说明**：本课实现的正弦位置编码（`sinusoidal_positions` / `pos_emb`）已在**第 20 课被 RoPE 取代**——
...
在 `Transformer::new` 里把它变成一个 `[block_size, n_embd]` 的张量存起来：
...
`Transformer` 结构体里它的类型是普通 `Tensor` 而不是 `Tensor::param(...)`：
...
在 `Transformer::forward` 里，用 `gather_rows` 按位置取行，再和 token embedding 相加：
...
`src/layers.rs` 里的结构体定义与构造（`src/layers.rs:87-105`）：
...
`forward` 的实际实现只有一行：调用融合算子 `Tensor::layernorm`（`src/tensor.rs:1877`），一次算完归一化的前向与反向。

> **原理示意（等价的分步展开，非源码）**：融合算子内部做的正是下面这 5 步，展开写只是为了对照公式；当前源码里已经没有这段分步代码了。
...
> Transformer 选 LayerNorm 还有一个重要原因：它不引入"batch 内样本之间的相互依赖"，行为可预测、实现简单（公式就 5 步）。
...
```rust
// TransformerBlock::new（src/model.rs:100-104）
ln1: NormLayer::new(cfg.n_embd, LN_EPS, cfg.use_rmsnorm),
ln2: NormLayer::new(cfg.n_embd, LN_EPS, cfg.use_rmsnorm),
// Transformer::new（src/model.rs:352）
ln_f: NormLayer::new(n_embd, LN_EPS, cfg.use_rmsnorm),
```

注意类型是 `NormLayer` 而不是 `LayerNorm`：它是"`LayerNorm` / `RMSNorm` 二选一"的枚举（`src/layers.rs:159`），由配置 `cfg.use_rmsnorm` 决定用哪一种（默认 `true` 即 RMSNorm；改回 `false` 才是本课讲的 LayerNorm，见第 21 课）。三个归一化层都在**层的最前面**（pre-norm 结构，下一课细讲），`eps` 统一用常量 `LN_EPS = 1e-5`（`src/model.rs:24`）。
...
fn forward(
    ...
    let h = self.mlp.forward(&h);   // MLPEnum：GELU MLP 或 SwiGLU MLP
    let h = if self.dropout > 0.0 { h.dropout(self.dropout, training) } else { h };
    x.add(&h)            // ← 残差连接 ②：x + MLP(LN(x))
}
```

> 前馈子层的字段是 `mlp: MLPEnum`（内部再选 GELU MLP 或 SwiGLU MLP），源码里**没有** `mlp_linear1` / `mlp_linear2` 这两个字段。
...
| | pre-norm（经典风格，我们用这个） | post-norm（原版 Transformer） |
|--|-----------------------------------|------------------------------|
| 残差路径上的数据 | 直接是"干净"的 x，不做归一化 | 每层都过 LN，数值被反复缩放 |
| 梯度传播 | 恒等路径无干扰，更稳 | 更容易出现训练不稳 |
| 深层堆叠 | 更容易训练（现代 LLM深度更大也能训） | 需要 warmup 等技巧 |
...
- pre-norm（LN → 子层 → 残差）是现代 LLM的标准结构

- 下一课：把这些积木全部拼起来，组成完整的 Transformer 模型！
- （本课的正弦位置编码在第 20 课被 RoPE 取代，见 [20-RoPE旋转位置编码.md](20-RoPE旋转位置编码.md)；LayerNorm 与残差连接保留至今。）
