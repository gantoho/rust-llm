...
`forward` 的实际实现只有一行：调用融合算子 `Tensor::layernorm`（`src/tensor.rs:1404`），一次算完归一化的前向与反向。
...
// GPT::new（src/model.rs:352）
ln_f: NormLayer::new(n_embd, LN_EPS, cfg.use_rmsnorm),
```

注意类型是 `NormLayer` 而不是 `LayerNorm`：它是"`LayerNorm` / `RMSNorm` 二选一"的枚举（`src/layers.rs:159`），由配置 `cfg.use_rmsnorm` 决定用哪一种（默认 `false`，即本课讲的 LayerNorm）。三个归一化层都在**层的最前面**（pre-norm 结构，下一课细讲），`eps` 统一用常量 `LN_EPS = 1e-5`（`src/model.rs:24`）。
