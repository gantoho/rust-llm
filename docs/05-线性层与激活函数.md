...
> 代码位置：
> - [src/layers.rs](../src/layers.rs)（Linear / LayerNorm / RMSNorm / MLP / LoRA / Embedding 等层，以及 GELU / Tanh 激活）
> - [src/module.rs](../src/module.rs)（Module trait 参数管理）
> - [src/rng.rs](../src/rng.rs)（xorshift64 随机数生成器，权重初始化用）
>
...
let mut opt = SGD::new(0.5, params);   // 优化器拿到全部 22 个参数（step() 需要 &mut self）
```
...
`layers.rs` 只提供了两个激活函数包装（`src/layers.rs:490`、`:495`），它们转调第 4 课 `tensor.rs` 里的张量方法：
...
ReLU **没有**这层包装函数，它是 `Tensor` 的方法：`Tensor::relu`（`src/tensor.rs:963-987`），直接写 `x.relu()` 即可（本项目第 7 课的演示实际用的是 `tanh`）。
...
| GELU | 约 (-0.17, +∞) | 0.5x(1+tanh(√(2/π)(x+0.044715x³))) | 见 7.2 | 平滑，现代 LLM默认 |
| Tanh | (-1, 1) | (e^x−e^-x)/(e^x+e^-x) | 1−c² | 输出居中，两端饱和 |

> 补充：softmax 不在 `layers.rs` 里，它是 `Tensor` 的方法——`Tensor::softmax_last_dim`（`src/tensor.rs:1759`，把一组分数变成概率分布，内部先减最大值保证数值稳定）与 `Tensor::log_softmax_last_dim`（`src/tensor.rs:2454`），后者是 CrossEntropy 的一部分，下一课会用到。学习计划表里第 5 课原写的是"Sigmoid"，实际代码中用 GELU 取代了它（现代 LLM 中更常用），ReLU / Tanh 均已实现。

## 8. 运行与测试

```bash
cargo test     # 测试全部通过（含 relu 梯度、softmax、线性回归收敛等）
cargo run --release -- demo    # 演示 1：MLP 学习 XOR（第 7 课完整讲解）
```
...
1. 把 `Linear::new` 里的 `std` 改成 0.01 或 5.0，跑第 7 课的 demo_xor，观察收敛速度变化，解释原因。
...
5. （选做）查资料对比 xavier 与 kaiming 初始化，并解释为什么 Embedding 用 `N(0, 0.02)`（标准差为 0.02 的正态分布，不是均匀分布的 bound；见 `layers.rs:459-468` 的 `Embedding::new`）。

## 10. 本课总结

- **Linear**：y = xW + b，weight `[in, out]`、bias `[out]`，支持 2D / 3D 输入
- **初始化**：Xavier 正态 `N(0, √(2/(in+out)))`（见第 4 节），保证激活量级不随层数放大
