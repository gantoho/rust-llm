...
> 补充：softmax 不在 `layers.rs` 里，它是 `Tensor` 的方法——`Tensor::softmax_last_dim`（`src/tensor.rs:1343`，把一组分数变成概率分布，内部先减最大值保证数值稳定）与 `Tensor::log_softmax_last_dim`（`src/tensor.rs:1877`），后者是 CrossEntropy 的一部分，下一课会用到。学习计划表里第 5 课原写的是"Sigmoid"，实际代码中用 GELU 取代了它（GPT 时代更常用），ReLU / Tanh 均已实现。
