...
| 自动微分 | `src/autograd.rs` | 线性 tape + `record` 登记反向闭包，`backward` 逆序扫描（计算图 → 梯度流） |
...
2. 构建 Transformer 模型（参数量由 `config/config.json` 的 `model` 段决定）
...
# 小模型（秒级完成，适合快速验证）：n_embd=64, n_layer=2, block_size=32（TransformerConfig 的默认值）
...
**每轮都会说明收尾原因**：正常回答在终端和日志里带 `（收尾：采到 EOS / 命中停止标记「…」/ 跑满 --max-new 被截断）`；
生成段为空白时打印 `[本轮无输出] …`，说清是"模型自己收了尾"还是"被截断"——空输出与"程序根本没跑"
在终端上是同一行空白，不给原因就无从判断。

**未正常收尾的回答不写进对话历史**：空回答整轮跳过；被 `--max-new` 截断（没有收尾标记）的回答也跳过，
并打印一条 `[info]`。这两种状态在训练语料里从未出现过——历史里一旦留下「助手：」后面空着，
或"说了 200 个 token 还不收尾"的示范，模型会跟着沉默，此后每轮全空。
...
| `--steps <步数>` | int | 按 SFT 语料推导：`min(config 的 train.steps, 400 × steps_per_epoch)` | 微调步数（`steps_per_epoch` = 语料 token 数 ÷ 每批 token 数）；显式给出就完全按它跑，不做任何收敛 |
...
**步数上限、评估间隔、预热步数都按 SFT 语料自己的 epoch 推导**：配置里的 `steps`（4000）和
`eval_every`（200）是配 471.9 万字预训练语料定的。SFT 语料常常只有一两万字、`steps_per_epoch`
只有几步，直接套这两个数会同时出问题：

- `steps = 4000` 等于上千个 epoch。实测 1.4 万字语料训 2600 步时 train 2.68 / val **7.77**，
  而 val 早在 600 步就见底了——多出来的步数全是过拟合；
- `eval_every` 收到 200 步后，两次评估之间隔了约 80 个 epoch，val 曲线上总共十几个点，
  "刚开始像样"的那一步很可能整个被跨过去，`best.ckpt` 就落在这么粗的网格上。

所以实际取 `eval_every = min(eval_every, 20 × steps_per_epoch)`（每 20 个 epoch 评估一次）、
`steps = min(steps, 400 × steps_per_epoch)`（早停的 patience 10 次评估 ≈ 200 个 epoch 会先到，
上限只是兜底）、`warmup_steps = min(warmup_steps, steps/10).max(1)`（预热太长会让大部分步数
都耗在爬坡上）。实际取值会打印在训练日志里。

**`best.ckpt` 与 `final.ckpt` 都要试**：SFT 语料通常很小（本项目 89 段对话 / 14233 token，
切 10% 当验证区），验证区与训练区同分布，于是 val 曲线往往**几十个 epoch 就见底**，之后
train_loss 继续降而 val_loss 回升——`best.ckpt` 是最低点，`final.ckpt` 是过拟合更深的一份。
实测同一组 6 轮输入：`best.ckpt`（val 6.7354 @ step 400）6/6 轮非空、全部由 EOS 收尾；
`final.ckpt`（step 1200，早停触发）同样 6/6 非空，句子更短、更像应答。两个都在输出目录里，各聊几句再定。

> **别用 val 已经回升到最差的那份 `final.ckpt`**：实测这种权重聊到第 3 轮起会连续输出空白——
> 它过拟合到"开口就吐 EOS"。判断依据是 `sft.csv` 里的 `val_loss` 列，不是文件名。
...
- 不带角色标记的文本整体跳过。
- 每段对话末尾的**收尾符号由程序自动补**，不用自己写：分词器带特殊 token 时补的是可训练的
  **EOS**；老分词器（没有特殊 token）才退化为文本标记 `。。`。

**模板标记要用预训练语料里"高频"的字，光"出现过"远远不够**（自定义模板时同样适用）。角色标记
（`用户：` / `助手：`）是每段对话都要被监督的**上下文**，模型得能原样续上；收尾符号更是每段回答的
监督目标，模型必须有本事把它生成出来，所以判据是**出现频率**而不是"出现过"。实测 `data/corpus_zh/`
（471.9 万字）：
...
模型学到的只是"回答完吐一个 `（`"，之后全无先验，输出被 `（结…` 这类括号乱码淹没。当时先改成
`。。`（`。` 是最高频字符，而 `。。` 组合在 471.9 万字里只出现 **2 次**，不与正文撞车），但那是
**靠语料里碰巧出现的字符组合去暗示"该收尾了"**，本质仍是碰运气。现在的做法是给分词器加
**EOS 特殊 token**：它可训练、每段回答都拿它当监督目标，模型学到的是明确的"说完了"，
不再依赖任何字符组合——所以模板是 `用户：` / `助手：` + EOS，而不是 `### 用户：` 这种
Markdown 风格标记，也不再需要 `。。`（它只对老分词器训出来的权重有意义，见
[`SFT_STOP`](file:///d:/Code/Rust/llm_from_scratch/src/main.rs) 的兜底注释）。

**示例**：

```bash
# ── 基础 SFT（步数/评估间隔按语料 epoch 推导，学习率取 config lr 的 1/10）──
cargo run --release -- sft --config config/config.json --pretrained checkpoints/zh/best.ckpt
...
| 预设 | 参数量 | 架构 | 适合场景 |
|------|--------|------|----------|
| `small` | ~3.3M | 经典风格（4层，256维，block=128，BPE 512） | 快速验证，CPU 几分钟 |
...
3. **训练小 Transformer 并生成文本**（第 12-20、25 课）：669 字符英文故事上训练 600 步，每 100 步记录一次
   （loss `1.30 → 0.16`），然后用 temperature=0.8 / top-k=10 / top-p=0.9 做三次生成：
...
```bash
# ── 标准演示（CPU）──
# 依次运行：XOR → BPE → 小 Transformer 训练+生成 → （无 GPU 提示）
cargo run --release -- demo
...
- 模型：`n_layer=2 n_embd=128 n_head=4 block_size=64`，词表来自内置小语料（字符级，35 个字符 + BOS / EOS / PAD 共 38 个 token）
...
=== 性能基准（bench）===
rayon 线程数：8
模型：n_layer=2 n_embd=128 n_head=4 block=64 vocab=38 | 参数 533120
开始训练：char（vocab=38）模型参数 533120 | 语料 671 tokens（训练 671 / 验证 0）| batch=4 block=64
step    30 | lr 0.000060 | loss 2.6754 | 1291 tok/s
[bench] train     : 30 steps | 5.954s | 0.1985s/step | 1290 tok/s
[bench] infer/kv  : 47 tok | 0.0909s | 517.0 tok/s
[bench] infer/full: 24 tok | 0.2230s | 107.6 tok/s
（以上 tok/s 越高越好；优化前后同机对比即可看出收益）
```

> 同一 seed 下每步 loss 与训练步数完全一致（上例 step 30 的 loss 恒为 `2.6754`），
> 因此数值正确性可以直接用 loss 对比来验证。`tok/s` 随机器而变，只用于**同机前后对比**。
...
```bash
cargo run --release -- quant [参数]        # 第 33 课：权重量化（RTN / Hessian 量化 / AWQ）
...
| 子命令 | 关键参数 | 输出分节 |
|--------|---------|---------|
| `quant` | `--bits int8\|int4`、`--method rtn\|hess\|awq`、`--calib-file` / `--calib-samples` / `--calib-tokens`、`--act-order`、`--damp`、`--alpha`、`--eval`、`--out` | 逐层量化误差报告（各算法对照）→（`--eval`）量化前后验证集 loss / 困惑度 → 量化权重落盘 |
...
| `n_embd` | int | `64` | 隐藏维度。经典 base 用 768，LLaMA-7B 用 4096 |
...
```jsonc
{
  "model": {
    ...
    "use_rmsnorm": true,    // LLaMA 风格（RMSNorm + SwiGLU，默认；false = 经典风格）
    "use_swiglu": true,
    "dropout": 0.1
  },
  "train": {
...
- **`cargo check --tests` 零警告**（含单测的编译无任何 warning）；**`cargo build` 与 `cargo build --features gpu` 编译通过、无 error**：
  ...
  `rag.rs` 的 `Retriever::from_text`、`rope.rs` 的 `with_window`）——
...
- 测试覆盖：自动微分、广播、softmax、BPE 编解码、RoPE 正交性与梯度、KV cache 与全量前向一致性、
  ...
  **量化（逐元素 / 逐通道 / 逐 token 量化与反量化、位打包往返、Hessian 量化分块不改结果、AWQ α 搜索、校准集合并、Hessian 逆的数值校验）**、
...
2. **可校验正确性**：固定种子下每步 loss 完全确定（如 step 30 恒为 `2.6754`），
...
- `Tensor::flash_attention` 的 CPU 路径已是分块在线 softmax（见 [第 24 课](docs/24-Flash-Attention.md)），
  前向中间 P 不落地、掩码核内屏蔽不物化；但反向仍要临时重算 scores/dP/dS，
  反向工作集是 **O(T²)**：长上下文时瓶颈已从前向转到反向
...
- Beam Search（`sample::beam_search`）已启用 KV cache：先对 prompt 做一次 prefill，
  之后每条路径 fork 一份独立缓存、每步只对 1 个新 token 前向；未启用缓存时回退为
  全量重算（`beam_size × max_new` 次前向）。打分按 `log_softmax` 后累加对数概率
  （此前累加原始 logit，既没归一化、又让长度惩罚方向与文档相反，已修正）。
  剩余开销是每扩展一个候选就把整段缓存深拷贝一份（兄弟路径前缀相同也各拷一份）
...
- **BPE 编码复杂度**：`BPETokenizer::encode` 若"每次只合并一个 pair 并全量重扫"是 O(n²×m)，
  大语料会卡死。改为经典风格的"按规则优先级单趟扫描替换"（O(len×合并数)），174KB 语料秒级编码。
...
- **SFT 模板标记要用语料里"高频"的字，"出现过"不够**：`#` 在 4.7M 字的中文语料里只出现 2 次，
  ...
  当时先换成 `。。`（`。` 是最高频字符，13.2 万次；`。。` 组合在语料里只出现 2 次），但那只解决了
  "别和正文撞车"，根子上仍是**靠语料里碰巧的字符组合暗示收尾**。最终解法是给分词器加
  **EOS 特殊 token**（词表 8192 → 8195，多出 BOS/EOS/PAD）：它可训练、每段回答都以它收尾，
  模型学到的是明确的停止信号。代价是特殊 token 之后建立的 checkpoint 与之前的 embedding 行数
  不再通用——换分词器就得重训。
- **聊天时"没输出"和"程序没跑"必须能分辨**：模型可能一个字都没说就收尾，而终端上那只是一行空白。
  `generate_with_reason` 明确交出三类收尾原因（EOS / 文本停止标记 / 跑满 `--max-new`），
  `chat` 每轮都把它记进日志；只有拿到这个信息，才分得清"权重过拟合到开口就吐 EOS"和"截断的回答
  污染了历史"——两者的修法完全不同（换 `best.ckpt` vs. 不把截断回答写回历史）。
...
- **量化部署**（第 33 课）→ `src/quant.rs`（INT8/INT4、Hessian 量化、AWQ、校准、逐层量化）+ `quant` 子命令
